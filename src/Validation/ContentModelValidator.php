...
final class ContentModelValidator extends AbstractTreeValidator
{
    ...
    protected function validateElement(ElementNode $node): void
    {
        ...
        /*
        |--------------------------------------------------------------------------
        | Interactive control nesting rule (corrected)
        |--------------------------------------------------------------------------
        */
        if (
            ...
            throw new ValidationException(
                "Interactive control <{$tag}> must not be nested inside another interactive control."
            );
        }
        ...
        */
        ...
        if ($tag === 'html') {
            if ($htmlDepth > 0) {
                throw new ValidationException(
                    "Nested <html> elements are not allowed."
                );
            }
            $htmlDepth++;
        }
        ...
        if ($tag === 'head') {
            ...
            if ($headCount > 1) {
                throw new ValidationException(
                    "Only one <head> element is allowed per document."
                );
            }
        }
